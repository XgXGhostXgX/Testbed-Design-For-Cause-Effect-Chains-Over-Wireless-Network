...
uint32_t uxPortResolveHostnameSilent(const char *pcHostname) {
  uint32_t uxAddress = 0;
  qcom_dnsc_get_host_by_name((char *)pcHostname, &uxAddress);
  return uxAddress;
}

int xIsValueFailed(int32_t xValue, int32_t xFailValue,
                   const char *pcFunctionName) {
  return (xValue == xFailValue);
}

void vPortSyslogSend(const char *pcDestination, const char *pcMessage) {
  int32_t xSocket = 0;
  SOCKADDR_T localAddr;
  char *pcMessageData = NULL;
  memset(&localAddr, 0, sizeof(localAddr));
  localAddr.sin_family = ATH_AF_INET;
  localAddr.sin_addr.s_addr = uxPortResolveHostnameSilent(pcDestination);
  //PORT
  localAddr.sin_port = 9000;

  // get socket
  xSocket = qcom_socket(ATH_AF_INET, SOCK_DGRAM_TYPE, 0);
  PRINTF("SEND MESSAGE TO: %c \r\n",*pcDestination);
  PRINTF("MESSAGE: %c\r\n",*pcMessage);
  PRINTF("SOCKET: %d \r\n",xSocket);
  if (xIsValueFailed(xSocket, -1, "qcom_socket")) {
    return;
  }

  // calculating full message length
  uint32_t uxMessageLength = strlen(pcMessage);
  pcMessageData = custom_alloc(uxMessageLength + 1);

  // assembling message
  memset(pcMessageData, 0, uxMessageLength + 1);
  memcpy(&pcMessageData[0], pcMessage, uxMessageLength);

  // send message
  qcom_sendto(xSocket, pcMessageData, uxMessageLength + 1, 0,
              (struct sockaddr *)&localAddr, sizeof(localAddr.sin_addr.s_addr));

  // tidy up
  custom_free(pcMessageData);
  pcMessageData = NULL;
  qcom_socket_close(xSocket);
}

char readUdpMessage(void){
    char message;
	//PRINTF("\r\n RecvBuf: %c", *udpRecvBuf);
    for(int i=1;i<udpRecvBufLen;i++){
    	//PRINTF("%c",*(udpRecvBuf+i));
    }
    //PRINTF("\r\n");
    message = *udpRecvBuf;
    //PRINTF("\r\n");
